...
import { helpAndSupportIssues } from "../data";
import { useAuth } from "../context/AuthContext";

const HelpSupport = ({ isOpen, onClose, onSubmit }) => {
  const { user } = useAuth();
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/40 backdrop-blur-sm p-4 overflow-y-auto">
      <div className="bg-white w-full max-w-lg p-6 rounded-xl shadow-lg relative border border-gray-200">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <select
              ...
              required
              className="w-full mt-1 px-3 py-2 border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:outline-none"
            >
              <option value="">Select Category</option>
              {Object.keys(helpAndSupportIssues).map((cat, idx) => (
          ...
          {formData.category && (
            <div>
              ...
              <select
                ...
                onChange={handleChange}
                required
                className="w-full mt-1 px-3 py-2 border border-gray-300 focus:ring-2 focus:ring-green-500 focus:outline-none"
              >
                <option value="">Select Problem</option>
                {helpAndSupportIssues[formData.category]?.map((problem, idx) => (
