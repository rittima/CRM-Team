        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {sortedTasks.map((task) => (
            <tr key={task._id} className="hover:bg-gray-50 transition">
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{task.description || "No description"}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{task.assignedBy ? "HR" : task.createdByName || "System"}</td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{task.createdAt ? new Date(task.createdAt).toLocaleString() : "-"}</td>

              {/* Status column only for HR tasks */}
              {task.assignedBy && (
